...
essential_columns = ['place_id', 'place_name', 'average_rating', 'granular_category', 'lat', 'lng']
...
def evaluate_model(vw_model, test_data, test, k=5):
    ...
    if not predictions or not actuals or not place_ids:
        logging.error("Evaluation failed: no predictions.")
        return None
    ...
    r2_val = rsquared(rating_true, rating_pred, "user_id", "item_id", "rating", "prediction")
    ndcg = ndcg_at_k(rating_true, rating_pred, k)
    precision = precision_at_k(rating_true, rating_pred, k)
    recall = recall_at_k(rating_true, rating_pred, k)
