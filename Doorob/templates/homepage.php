...
function toggleRating() {
  ...
  <!-- CF error -->
  <section class="product" id="cf1-message" style="display: none;">
<h3 class="product-category">Recommended Destinations Based on What Others Like</h3>
</section>
<!-- CF results -->
<section class="product" id="cf1-section" style="display: none;"> 
    <!--Recommended Destinations Based on What Others Like--> 
    ...
    <button class="pre-btn"><img src="imgs/arrow.png" alt=""></button>
    <button class="nxt-btn"><img src="imgs/arrow.png" alt=""></button>
    <div class="product-container" id="CF1product-container">
    </div>
</section>

<!-- CX error -->
<section class="product" id="cx1-message" style="display: none;">
<h3 class="product-category">Best Nearby Destinations</h3>
</section>
<!-- Context -->
<section class="product" id="context1-section" style="display: none;"> 
    <!--Best Places for You Based on Your Location --> 
    ...
    <button class="pre-btn"><img src="imgs/arrow.png" alt=""></button>
    <button class="nxt-btn"><img src="imgs/arrow.png" alt=""></button>
    <div class="product-container" id="CX1product-container">
    </div>
</section>

<!-- Hybrid -->
<!-- hybird error -->
<section class="product" id="hybrid1-message" style="display: none;">
<h3 class="product-category">Top Recommended Destinations for You </h3>
</section>
<!-- Hybrid -->
<section class="product" id="hybrid1-section" style="display: none;"> 
    <!--Personalized Destinations Just for You-->
    ...
    <button class="pre-btn"><img src="imgs/arrow.png" alt=""></button>
    <button class="nxt-btn"><img src="imgs/arrow.png" alt=""></button>
    <div class="product-container" id="HY1product-container">
    </div>
</section>
...
$(document).on('click', '.favorite-btn', function() {
    ...
    function showDetails(placeId) {
        ...
        document.getElementById('foort').style.visibility = 'hidden';
        document.getElementById('cx-message').style.visibility = 'hidden';
        document.getElementById('cf-section').style.visibility = 'hidden';
        document.getElementById('hybrid-message').style.visibility = 'hidden';
        document.getElementById('hybrid-section').style.visibility = 'hidden';
...
.then(() => {
renderCXPlaces();
document.getElementById('context1-section').style.display = 'block';
})
.catch(error => {
...
fetch(`http://127.0.0.1:5001/api/recommendations/<?php echo $user_id; ?>?category=${encodeURIComponent(selectedCategory)}`)
  ...
  .then(() => {
    ...
    document.getElementById('cf1-section').style.display = 'block';
  })
  .catch(error => {
...
if (recommendations && recommendations.length > 0) {
    // Show the CF section if there are recommendations
    const cfSection = document.getElementById('cf1-section'); // Get the CF section by ID
    if (cfSection) {
        cfSection.style.display = 'block'; // Make the CF section visible
    }
} else {
    // Show a message to the user if there are no recommendations
    const messageContainer = document.getElementById('cf1-message'); // Container for the message
    if (messageContainer) {
        messageContainer.innerHTML = '';
...
const cfrsPlaceImageCache = {};
function renderCFRSPlaces() {
    const cfrsPlacesContainer = document.getElementById('CF1product-container');
    cfrsPlacesContainer.innerHTML = ''; // Clear current recommendations
...
if (context_recommendations && context_recommendations.length > 0) {
    // Show the CF section if there are recommendations
    const cxSection = document.getElementById('context1-section'); // Get the CF section by ID
    if (cxSection) {
        cxSection.style.display = 'block'; // Make the CF section visible
    }
} else {
    // Show a message to the user if there are no recommendations
    const messageContainer = document.getElementById('cx1-message'); // Container for the message
    if (messageContainer) {
        messageContainer.innerHTML = '';
        messageContainer.innerHTML = ` <h3 class="product-category">Best Nearby Destinations</h3>`;
...
const cxPlaceImageCache = {};
function renderCXPlaces() {
    const cxPlacesContainer = document.getElementById('CX1product-container');
    cxPlacesContainer.innerHTML = ''; // Clear current recommendations
    // <p class="product-short-description">Category: ${place.granular_category}</p>
    // Display the next set of recommendations (3 at a time)
    for (let i = currentIndexCx; i < context_recommendations.length; i++) {
...
// Check if recommendations have data
if (hybrid_recommendations && hybrid_recommendations.length > 0) {
    const HybridSection = document.getElementById('hybrid1-section');
    if (HybridSection) {
        HybridSection.style.display = 'block'; // Show section if recommendations exist
    }
} else {
    // Show a message to the user if there are no recommendations
    const messageContainer = document.getElementById('hybrid1-message'); // Container for the message
    if (messageContainer) {
        // Create a new div for the error message
        const errorDiv = document.createElement('div');
        errorDiv.className = 'errormassage'; // Add the class for styling
...
function renderHybridPlaces() {
    const HYPlacesContainer = document.getElementById('HY1product-container');
    HYPlacesContainer.innerHTML = ''; // Clear previous recommendations
...
function closeModal() {
    ...
    document.getElementById('intro-Container').style.display = 'grid';
        ...
        document.getElementById('cx-message').style.visibility = 'visible';
